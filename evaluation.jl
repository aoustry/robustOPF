...
function EvalXbatch(model, ev, input_bar,sizebatch,pl,ql)
    """Input bar est le vecteur d'initalisation de la méthode de Newton """
    ...
    for i in 1:sizebatch
        ...
        ev.stack.pload[1:nbus] = pl[:,i] ;
        ev.stack.qload[1:nbus] = ql[:,i] ;
        ### Load Flow
        ExaPF.set_params!(ev.jac, ev.stack);
        ExaPF.nlsolve!(ev.solver, ev.jac, ev.stack)
        array = vcat(array,ev.stack.input[map_x])
    end
    return array
end
